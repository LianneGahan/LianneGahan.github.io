...
from latest.Code.simulation_main import simulation_run_analysis, simulation_setup
...
# Still needs to be added as an option in the GUI

# Compare to gridsearch 
# Write best conditions to file and a euclidian distance
compare_to_gridsearch("gridsearch.json", "bond_distribution.json") 
# Simply run a library generation for those parameters, the fitting aspect has been removed for simplicity. If needed, start again using 
# the original ligninfit on github at liannegahan. There "evo_wrapper.sh" acts as the main, and the file structure is a bit more complex.


# Simulation run with the given conditions
SG_ratio, simulation_time, monomer_addition_rate, minimum_num_monomers, maximum_num_monomers, rxn_rates, number_of_runs, filewriting, filepath = simulation_setup()
...
if full_library:
    print("Library ready, please find at 'output/library.json'")
else:
    # run molecule selection molecule_selection(N_structures, DP, DP_range)
    print("Molecule selection complete")